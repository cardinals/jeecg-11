...
//					upload(rootPath + relativePath, fileName,id,businessId,projectId,phasesId,itemsId,materialsId,type);
					upload(relativePath, fileName,id,businessId,projectId,phasesId,itemsId,materialsId,type);
				} else {
...
//	}
	...
	public void upload(String filePath,String realName,String id,String businessId,String projectId,String phasesId,
		...
		if(StringUtil.isNotEmpty(id) && StringUtil.isNotEmpty(file.getMaterialsPath())){
			...
			String getIdSql ="select  func_getnewid(20) as id from dual";
			List<Map<String, Object>> resultId =  systemService.findForJdbc(getIdSql);
			String getId = resultId.get(0).get("id")+"";
...
//					", '"+file.getMaterialsId()+"', '', '1', '', '', '', '',  '"+file.getMaterialsName()+"','"+file.getMaterialsType()+"')";
			String sql ="insert into a_materials_upload" +
					"  (id, business_id, project_id, phases_id, items_id, materials_id, materials_path, status, create_time, update_time, remark, create_by, materials_name, materials_type)" +
					"values  ('"+getId+"', '"+file.getBusinessId()+"', '"+file.getProjectId()+"', '"+file.getPhasesId()+"', '"+file.getItemsId()+"'" +
					", '"+file.getMaterialsId()+"', '', '1', '', '', '', '',  '"+file.getMaterialsName()+"','"+file.getMaterialsType()+"')";
			int result =  systemService.updateBySqlString(sql);
			System.out.println("#########insert sql :"+sql);
			file = aMaterialsUploadService.getEntity(AMaterialsUploadEntity.class, getId);
		}
...
//		TagUtil.datagrid(response, dataGrid);
		...
		String condition = "";
		if (StringUtil.isNotEmpty(aMaterialsUpload.getBusinessId())) {
			condition += " and a.business_id ='" +aMaterialsUpload.getBusinessId()+"'";
		}
		if (StringUtil.isNotEmpty(aMaterialsUpload.getMaterialsType())) {
			condition += " and a.materials_type ='" +aMaterialsUpload.getMaterialsType()+"'";
		}
		if (StringUtil.isNotEmpty(aMaterialsUpload.getMaterialsName())) {
			condition += " and substr(a.materials_name ,37) ='" +aMaterialsUpload.getMaterialsName()+"'";
		}
		if (StringUtil.isNotEmpty(aMaterialsUpload.getItemsId())) {
			condition += " and b.items_child_name ='" +aMaterialsUpload.getItemsId()+"'";
		}
		String sql = "select a.id,a.business_id,a.materials_id,a.materials_path,a.update_time," +
				...
				"       a.materials_type," +
				"       b.items_child_name as items_id" +
				"  from A_MATERIALS_UPLOAD a, a_items_info b " +
				" where a.items_id = b.items_id||b.items_child_id " +condition +
		...
		if(resultList!=null && resultList.size()>0){
			for(int i=0;i<resultList.size();i++){
				...
				n.put("itemsId",String.valueOf(obj.get("items_id")));
